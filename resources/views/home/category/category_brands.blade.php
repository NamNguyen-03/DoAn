...
<div class="col-sm-9 padding-right">
    <div class="features_items" style="min-height: 300px;"><!--features_items-->
        <h2 class="title text-center" id="product_title"></h2>
        <div class="brand-slider-container">
            <div id="category_brand" class="brand-slider">
                <!-- Các brand sẽ được chèn bằng JS -->
            </div>
        </div>
        <br>
...
<script>
    ...
    if (!categorySlug) {
        showAlert('Lỗi', 'Không tìm thấy Slug Category sản phẩm!', 'error', 'red');
    }
    if (!brandSlug) {
        showAlert('Lỗi', 'Không tìm thấy Slug Brand sản phẩm!', 'error', 'red');
    }

    // Gọi API để lấy sản phẩm theo category_id
    function fetchCateBrandProducts() {
        fetch(`/api/category/${categorySlug}?brand=${brandSlug}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data) {
                    allProducts = data.data;
                    document.getElementById("product_title").innerHTML = `<p>${data.category} - ${data.brand}</p>`;
                    renderProducts(allProducts); // Hiển thị sản phẩm
                    renderPagination(allProducts); // Hiển thị phân trang
                } else {
                    document.getElementById("product-list").innerHTML = "<span>Không có sản phẩm nào</span>";
                }
            })
            .catch(error => {
    ...
    function renderProducts(products) {
        ...
        productsToShow.forEach(product => {
            let productName = product.product_name.length > 50 ? product.product_name.substring(0, 50) + "..." : product.product_name;
    ...
    // Lắng nghe sự kiện khi nhấn nút "Xem kết quả"
    ...
    function renderFilteredProducts(products) {
        filteredProducts = products; // Lưu danh sách sản phẩm đã lọc
        renderProducts(filteredProducts); // Hiển thị các sản phẩm đã lọc
        renderPagination(filteredProducts); // Hiển thị phân trang cho sản phẩm đã lọc
    }
    // Tạo nút phân trang
    function renderPagination(products) {
        ...
        let totalPages = Math.ceil(products.length / productsPerPage); // Tổng số trang

        if (totalPages <= 1) return; // Nếu chỉ có 1 trang, không hiển thị phân trang
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            pageItem.addEventListener("click", function(event) {
                event.preventDefault();
                currentPage = i;
                renderProducts(products); // Hiển thị sản phẩm trang mới
                renderPagination(products); // Cập nhật nút phân trang
            });
    ...
    document.getElementById('clear-filter').addEventListener('click', function() {
        ...
        // Reset các range slider
        document.getElementById('range-min').value = 0;
        document.getElementById('range-max').value = 5000000;

        // Cập nhật lại các sản phẩm không có lọc
        renderProducts(allProducts);
        renderPagination(allProducts) // Gọi lại hàm renderProducts() để làm mới danh sách sản phẩm
    });
    document.addEventListener("DOMContentLoaded", () => {
        fetchCateBrandProducts();
        renderCategoryBrands(categorySlug);
    ...
    function renderCategoryBrands(categorySlug) {
        const container = document.getElementById('category_brand');
        container.innerHTML = '<div>Đang tải thương hiệu...</div>';

        fetch(`/api/category/${categorySlug}/brands`)
            .then(res => res.json())
            .then(data => {
                if (!data.length) {
                    container.innerHTML = '<div>Không có thương hiệu nào.</div>';
                    return;
                }
                ...
                data.forEach(brand => {
                    ...
                    item.addEventListener('click', () => {
                        window.location.href = `/category/${categorySlug}/brand/${brand.slug}`;
                    });
            ...
            .catch(() => {
                container.innerHTML = '<div>Lỗi tải thương hiệu.</div>';
            });
    }
